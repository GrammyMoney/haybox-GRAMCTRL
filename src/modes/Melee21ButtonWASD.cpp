...
Melee21ButtonWASD::Melee21ButtonWASD(socd::SocdType socd_type, Melee21ButtonWASDOptions options) {
    _socd_pair_count = 4;
...
void Melee21ButtonWASD::HandleSocd(InputState &inputs) {
    _horizontal_socd = inputs.left && inputs.right;
    InputMode::HandleSocd(inputs);
}

void Melee21ButtonWASD::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;
    outputs.x = inputs.x;
    outputs.y = inputs.y;
    ...
    // Activate D-Pad layer by holding Mod X + Mod Y or Up button.
...
void Melee21ButtonWASD::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
